...
# Matrix Character gradient (from low/deep to high/foam)
# Using code-like symbols: binary, operators, brackets, hex
...
# Generate color palette (Gradient: Black -> Dark Green -> Matrix Green -> White)
def generate_palette(steps):
    # Keyframes: (position 0.0-1.0, (r, g, b))
    stops = [
        (0.0, (0, 0, 0)),  # Void
        (0.3, (0, 30, 0)),  # Deep Code
        (0.6, (0, 120, 30)),  # Mid Green
        (0.8, (0, 255, 70)),  # Bright Matrix Green
        (1.0, (200, 255, 200)),  # White Phosphorus
    ]
    ...
    for i in range(steps):
        ...
        for j in range(len(stops) - 1):
            if stops[j][0] <= t <= stops[j + 1][0]:
                ...
                # For Matrix look, keep background mostly black/very dark green
                bg_r, bg_g, bg_b = 0, g // 8, 0
...
def main():
    ...
    try:
        while True:
            ...
            # 3. Detail Ripples (Faster) - increased frequency for "digital" noise
            Z += 0.2 * np.sin(X * 3.0 + t * 3.0)
        ...
        clear_screen()
        show_cursor()
        print("Matrix Ocean simulation ended.")
